...
module.exports = {
	...
	initialize: function() {
		...
		bot.on('messageReactionRemove', (reaction, user) => {
			try {
				let output = "n00b " + user.username + " ashamed of emoji given";
				bot.channels.cache.get(reaction.message.channelId).send(output);
			} catch (error) {
				console.log('DISCORD REACTION REMOVE CALLOUT FAILURE');
				console.log(error);
			}
		});
